...
void nouvellepartie() {
    ...
    if (getcwd(cwd, sizeof(cwd)) != NULL) {
        printf("Repertoire de travail courant : %s\n", cwd);
    } else {
        perror("getcwd");
    }
    int nombredejoueur;
    ...
    while (!finDePartie) {
        ...
        printf("Verification de fin de partie pour le joueur %d (%s) à la position (%d, %d)\n",
        ...
        printf("finDePartie apres verification : %d\n", finDePartie);
...
void chargerpartie() {
    ...
    fclose(fichier);
    printf("Partie chargee avec succes depuis le fichier %s.\n", nomFichier);
...
void sauvegarderPartie(CasePlateau plateau[TAILLE_PLATEAU][TAILLE_PLATEAU], Joueur joueurs[], int nombredejoueurs, int joueurCourant) {
    ...
    fclose(fichier);
    printf("Partie sauvegardee avec succes dans le fichier %s.\n", nomFichier);
}
...
void afficheraide() {
    ...
    printf("=== Aide du Jeu ===\n");
    printf("Le but du jeu est d'atteindre le cote oppose du plateau avant vos adversaires.\n");
    printf("Vous pouvez deplacer votre pion ou placer des barrieres pour bloquer vos adversaires.\n");
    printf("Vous avez 10 barriere en commencant le jeu, pour avancer il faut saisir les coordonnees x y ou vous voulez aller. \n ");
    printf("Pour placer des barrieres il faut saisir les coordonnees x y d ou vous voulez les placer.\n");
...
void placerBarriere(Joueur *joueur, CasePlateau plateau[TAILLE_PLATEAU][TAILLE_PLATEAU]) {
    ...
    int x, y;
    char orientation;
    printf("Entrez les coordonnees de la case de depart pour la barriere (x y) :\n");
    scanf("%d %d", &x, &y);
    printf("Entrez l'orientation de la barriere (h pour horizontale, v pour verticale) :\n");
    scanf(" %c", &orientation);
